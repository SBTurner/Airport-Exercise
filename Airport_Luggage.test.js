...
describe('Tests on the aiport', ()=>{
    ...
    test('Plane has a name',()=>{
        const plane = new Plane("Jet2","Barbados")
        expect(plane.name).toBe("Jet2")
    })
    test('Sarah has boarded the plane',()=>{
        const person = new Person("Sarah")
        const plane = new Plane("Jet 2","Barbados")
        plane.boardPlane(person)
        expect(plane.boarded[0].name).toBe(person.name)
    })
...
describe("Airport Simulation", ()=>{
    test("Simulation 1",()=>{
        ...
        plane2.boardPlane(passenger2, passenger3)
        ...
        expect(airport_dest2.planes[0].boarded[1].bags[0].weight).toBe(2)
        console.log(airport_dest2.planes[0].boarded[1].bags[0].weight)
